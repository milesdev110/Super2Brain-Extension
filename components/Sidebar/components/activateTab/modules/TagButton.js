...
const TagButton = memo(({ tag, isFirstTag, isAiThinking, onTagClick, useInput }) => {
  ...
  return (
    ...
    >
      <tag.icon className={`w-4 h-4 ${tag.type === "screenshot" ? "-rotate-90" : ""}`} />
      <Tooltip
        style={{ borderRadius: "8px" }}
        anchorSelect={`.tags-button-${tag.type}`}
        place="top"
      >
        {tag.type === "bookmark" && !useInput ? "请先登录才能将网页导入知识库" : tag.text}
      </Tooltip>
    </button>
  );
});
