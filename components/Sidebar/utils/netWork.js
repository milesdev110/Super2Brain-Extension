...
import { createStreamRequest, handleStreamResponse } from "./streamUtils";
...
export const getNetwork = async (query, context = [], onProgress) => {
  ...
  const messages = buildMessages(getSystemPrompt(), context, searchResults, query);

  const { content: response } = await getFinalResponse(messages, onProgress);

  // 生成相关问题
  const similarQuestions = await generateSimilarQuestions(query, response, onProgress);
